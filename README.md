...
- **Serverless対応**: CDKで`Lambda + S3 + CloudFront`を一括デプロイ
...
- AWSアカウント（Serverlessデプロイ時）
- AWS CLI設定済み（Serverlessデプロイ時）
...
## AWS Serverless Deploy（CDK）

このプロジェクトは、次の構成をCDKで一括デプロイできます。

- Frontend: S3（private） + CloudFront
- Backend: Lambda Function URL（`/api/*`をCloudFront経由でルーティング）

### 1. CDKの初期化（アカウント/リージョン単位で初回のみ）
...
APIキーは CloudFormation パラメータ（NoEcho）で渡します。
...
デプロイ完了後、CloudFormation Outputに以下が表示されます。

- `CloudFrontUrl`（アプリURL）
- `ApiRoute`（`/api/generate` のURL）
...
├── lambda/
│   └── api-handler.js     # Lambda APIハンドラ
├── bin/
│   └── cdk.js             # CDKエントリポイント
├── lib/
│   └── llm-logprob-dashboard-stack.js  # CDKスタック定義
├── cdk.json               # CDK設定
